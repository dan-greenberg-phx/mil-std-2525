...
const ImageGallery = ({
  ...
  async function downloadSymbol() {
    ...
    let imgEl = document
      .getElementsByClassName("ReactGridGallery_tile-viewport")
      .item(0)
  ...
  function formatSymbolMetadata(sidc) {
    ...
    output +=
      "BaseAffiliation: " + msSymbol.getMetadata().baseAffilation + "\n";
  ...
  function ClickOffModalclose() {
    useEffect(() => {
      ...
      document.addEventListener("mousedown", (event) => {
        console.log("active: " + document.activeElement.id);
        if (
          !sidcModalRef.current.contains(event.target) &&
          document.activeElement.id !== "modalId" &&
          document.activeElement.id !== "sidcInput" &&
          document.activeElement.id !== "sidcButton"
        ) {
          console.log(
            "conatins: " + sidcModalRef.current.contains(event.target)
          );
